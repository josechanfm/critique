...
<AdminLayout>
    ...
    <div class="w-[1000px] mx-auto my-4 " v-if="!readingFinish">
        ...
        <div v-if="currentStage === 'guide'" class="guide flex flex-col gap-4">
            ...
            <div class="text-base py-2">请仔细阅读以下三篇文章，系统将记录您的阅读时间</div>
            <button @click="startReading" class="bg-blue-600 hover:bg-blue-700 w-32">开始阅读</button>
        </div>

        <!-- 文章阅读页 -->
        <div v-else class="rounded-lg shadow bg-white p-10">
            <h2>{{ currentArticle.title }}</h2>
            <div class="article-content text-wrap" v-html="currentArticle.content"></div>

            <div class="timer">
                阅读时间: {{ formatTime(readingTimes[currentArticleIndex]) }}
            </div>

            <button v-if="currentArticleIndex < articles.length - 1" @click="nextArticle" class="next-button bg-lime-600 hover:bg-lime-700">
                下一篇
            </button>
            <button v-else @click="finishReading" class="bg-amber-600 hover:bg-amber-700">
                完成阅读
            </button>
        </div>
    </div>
